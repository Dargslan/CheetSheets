# Ethtool Network Diagnostics

> Complete ethtool cheat sheet — check NIC speed, duplex, driver info, offload settings, ring buffers, RSS queues, Wake-on-LAN, self-test, and sysfs quick checks. Includes dargslan-ethtool-check CLI tool commands.

---

## Quick Reference

This cheat sheet provides a concise, printable reference for **Ethtool Network Diagnostics**.

For the complete guide with detailed explanations, examples, and best practices, visit the full article on our website.

---

## Resources

| Resource | Link |
|---|---|
| **Download PDF** | [https://dargslan.com/cheat-sheets/ethtool-network-diagnostics](https://dargslan.com/cheat-sheets/ethtool-network-diagnostics) |
| **Browse All Cheat Sheets** | [https://dargslan.com/cheat-sheets](https://dargslan.com/cheat-sheets) |
| **Browse 210+ IT Books** | [https://dargslan.com/books](https://dargslan.com/books) |

---

*This cheat sheet is part of the [Dargslan Cheat Sheet Library](https://dargslan.com/cheat-sheets) — free, professional IT reference materials for Linux, cybersecurity, DevOps, and more.*
